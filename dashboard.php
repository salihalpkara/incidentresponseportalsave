        ...
        <div class="col">
            <div class="card">
                <div class="card-header d-flex align-center">
                    <h3 class="me-auto mb-0 py-1">Latest Incident Reports</h3>
                        ...
                        <tbody id="myTable">
                            ...
                            <?php else: ?>
                                <?php if (count($incidents) >= 5): ?>
                                    <?php for ($i = 0; $i < 5; $i++): ?>
                                        <tr>
                                            <td><a href="view_report.php?id=<?= $incidents[$i]["incident_id"] ?>" class="d-block text-decoration-none text-light"><?= htmlspecialchars($incidents[$i]['incident_id']) ?></a></td>
                                            <td><a href="view_report.php?id=<?= $incidents[$i]["incident_id"] ?>" class="d-block text-decoration-none text-light"><?= htmlspecialchars($incidents[$i]['description']) ?></a></td>
                                            <td><a href="view_report.php?id=<?= $incidents[$i]["incident_id"] ?>" class="d-block text-decoration-none text-light"><?= htmlspecialchars($incidents[$i]['incident_date']) ?></a></td>
                                            <td><a href="view_report.php?id=<?= $incidents[$i]["incident_id"] ?>" class="d-block text-decoration-none text-light"><?= htmlspecialchars($incidents[$i]['type_name']) ?></a></td>
                                            <td><a href="view_report.php?id=<?= $incidents[$i]["incident_id"] ?>" class="d-block text-decoration-none text-light"><?= htmlspecialchars($incidents[$i]['severity_name']) ?></a></td>
                                            <td><a href="view_report.php?id=<?= $incidents[$i]["incident_id"] ?>" class="d-block text-decoration-none text-light"><?= htmlspecialchars($incidents[$i]['created_by_name']) ?></a></td>
                                            <td><a href="view_report.php?id=<?= $incident["incident_id"] ?>" class="d-block text-decoration-none <?php switch (htmlspecialchars($incidents[$i]['latest_status_name'])):
                                                                                                                                                        case "In progress": ?> text-warning <?php break; ?> <?php
                                                                                                                                                                                                        case "Resolved": ?> text-success<?php break; ?><?php
                                                                                                                                                                                                                                                    case "Pending": ?> text-danger<?php break; ?><?php endswitch; ?> "><?= htmlspecialchars($incidents[$i]['latest_status_name']) ?></a></td>
                                        </tr>
                                    <?php endfor; ?>
                                <?php else: ?>
                ...
                <div class="card-footer d-flex">
                    <a href="view_reports.php" class="btn btn-primary m-auto"><i class="bi bi-eyeglasses me-2"></i>View All Reports</a>
